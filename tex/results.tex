...
With that experience, and given the difficulty caused by sharp edges, we are satisfied with studying grid-independence here instead.
...
Based on the results of our grid-independence study, we decided to use triangle densities in the order of $6$ to $9\times10^{-5}$
to produce the meshes for the replication of figures 14a and 14b in the work of Rockstuhl et al.\ (case $a1=672$, $b=328$). For the third 
dimension, we needed to choose a value that represents ``infinity.'' To achieve this, we studied the effects of 
